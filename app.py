...
with open('text\suggestion.txt', 'r') as f:
    suggestions = f.read().splitlines()

    
def suggest_completions(input_text):
    completions = [suggestion for suggestion in suggestions if suggestion.startswith(input_text)]
    return completions
...
demo = gr.Interface(
    ...
    inputs=[
            ...
            gr.Dropdown(label="Prompt", choices=suggestions, interactive=True),
    ],
...
def process_img(image, prompt):
    # Use the suggest_completions function to generate a list of options based on the user's input
    options = suggest_completions(prompt)

    # If there are any options, use the first one as the prompt
    if options:
        prompt = options[0]

    # Process the image and prompt as before
    img = np.array(image)
    img = img / 255.
    img = img.astype(np.float32)
    y = torch.tensor(img).permute(2,0,1).unsqueeze(0).to(device)
